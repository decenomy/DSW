...
```
wget -q https://raw.githubusercontent.com/decenomy/BECN/master/contrib/masternodesetup/masternodesetup.sh
```
...
Remember to do `beacon-cli getblockcount` to check if VPS catching blocks till it synced with chain, if not follow this procedure:

* Go to your wallet-qt and check peers list (tools - peers list) and select one ip from the list. With that ip do the follow command at VPS `beacon-cli addnode "ip" onetry`

      Example:
		  beacon-cli addnode 45.32.144.158 onetry
    
* Check now if VPS already downloading blocks with the command `beacon-cli getblockcount`, and if yes give it time now to catch last block number 
...
## Congratulations your Beacon node it's running
