...
If you are running an older version, shut it down. Wait until it has completely shut down (which might take a few minutes for older versions), then run the installer (on Windows) or just copy over /Applications/Jackpot-Qt (on Mac) or pivxd/pivx-qt (on Linux).
...
-	Fixed a  bug that would not start masternodes from the Jackpot-Qt masternodes tab
-	Updated Jackpot-Qt tooltips
