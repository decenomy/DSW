...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all KYAN specific functionality (Masternodes, Zerocoin, Budgeting) (0-1, default: %u)"), 0));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //Kyanite specific: zerocoin and spork DB's
                ...
                if (fReindexMoneySupply) {
                    LOCK(cs_main);
                    // Skip zKYAN if already reindexed
