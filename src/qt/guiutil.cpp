...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter ESK address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no ESK: URI
...
{
    // Convert eskacoin:// to eskacoin:
    //
    //    Cannot handle this later, because eskacoin:// will cause Qt to see the part after // as host,
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "eskacoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for eskacoin.lnk
...
{
    return GetAutostartDir() / "eskacoin.desktop";
}
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the eskacoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add eskacoin app to startup item list
