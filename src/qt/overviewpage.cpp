...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::CFL)
    {
    }
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeCryptoFlowAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeCryptoFlowAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeCryptoFlowAmount = strAnonymizeCryptoFlowAmount.remove(strAnonymizeCryptoFlowAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCryptoFlowAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeCryptoFlowAmount * COIN) nMaxToAnonymize = nAnonymizeCryptoFlowAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeCryptoFlowAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeCryptoFlowAmount));
        strAnonymizeCryptoFlowAmount = strAnonymizeCryptoFlowAmount.remove(strAnonymizeCryptoFlowAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCryptoFlowAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizeCryptoFlowAmount == 0) {
