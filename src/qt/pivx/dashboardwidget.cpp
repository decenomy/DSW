    ...
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of BIR earned via Staking & Masternodes"));
	...
	ui->labelAmountMNRewards->setText("0 BIR");
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("BIR")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair BIR, zBIR
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(6,86,135));
	set1->setColor(QColor(240,186,0));
