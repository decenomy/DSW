...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("MobilityCoin client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("MobilityCoin"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("MobilityCoin"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
