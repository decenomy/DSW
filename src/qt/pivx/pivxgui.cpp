...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Birake client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Birake"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Birake"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
