...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Kyanite client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Kyanite"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Kyanite"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
