...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Dash Diamond client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Dash Diamond"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Dash Diamond"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
