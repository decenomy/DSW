...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\suverenoParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\suverenoParams
    // Mac: ~/Library/Application Support/suverenoParams
    // Unix: ~/.suvereno-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SuverenoParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "SuverenoParams";
#else
    // Unix
    return pathRet / ".suvereno-params";
#endif
#endif
}
