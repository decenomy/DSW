...
int nAnonymizeCryptoFlowAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "cryptoflow" is a composite category enabling all CryptoFlow-related debug output
            if (ptrCategory->count(string("cryptoflow"))) {
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "cryptoflow.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "cryptoflowd.pid"));
