...
UniValue importprivkey(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
        throw std::runtime_error(
            "importprivkey \"trittiumprivkey\" ( \"label\" rescan fStakingAddress )\n"
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (!seed_id.IsNull())
    {
        ...
        if (pwalletMain->GetKey(seed_id, seed)) {
            ...
            CExtKey cointypeKey;           //key at m/purpose'/coin_type'  --> key at m/44'/838'
            CExtKey accountKey;            //key at m/purpose'/coin_type'/account' ---> key at m/44'/838'/0'
            CExtKey changeKey;             //key at m/purpose'/coin_type'/account'/change ---> key at m/44'/838'/0'/HDChain::ChangeType::ECOMMERCE.
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        if (pwalletMain->GetKey(keyid, key)) {
            ...
            // FLS export
            strAddr = EncodeDestination(keyid, CChainParams::PUBKEY_ADDRESS_FLS);
            file << strprintf("%s %s ", KeyIO::EncodeSecret(key, CChainParams::SECRET_KEY_FLS), strTime);
            if (pwalletMain->mapAddressBook.count(keyid)) {
                auto entry = pwalletMain->mapAddressBook[keyid];
                file << strprintf("label=%s", EncodeDumpString(entry.name));
            } else if (keyid == seed_id) {
...
UniValue bip38encrypt(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "bip38encrypt \"trittiumaddress\" \"passphrase\"\n"
...
UniValue bip38decrypt(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "bip38decrypt \"trittiumaddress\" \"passphrase\"\n"
