...
UniValue importprivkey(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
        throw std::runtime_error(
            "importprivkey \"pepsprivkey\" ( \"label\" rescan fStakingAddress )\n"
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (!seed_id.IsNull())
    {
        ...
        if (pwalletMain->GetKey(seed_id, seed)) {
            ...
            CExtKey cointypeKey;           //key at m/purpose'/coin_type'  --> key at m/44'/847'
            CExtKey accountKey;            //key at m/purpose'/coin_type'/account' ---> key at m/44'/847'/0'
            CExtKey changeKey;             //key at m/purpose'/coin_type'/account'/change ---> key at m/44'/847'/0'/HDChain::ChangeType::ECOMMERCE.
...
UniValue bip38encrypt(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "bip38encrypt \"pepsaddress\" \"passphrase\"\n"
...
UniValue bip38decrypt(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "bip38decrypt \"pepsaddress\" \"passphrase\"\n"
