        ...
        if (zwallet) {
            ...
            if (CWalletDB(strWalletFile).ReadCurrentSeedHash(hashSeed)) {
                ...
                if (!GetDeterministicSeed(hashSeed, nSeed)) {
                    return error("Failed to read zSUV seed from DB. Wallet is probably corrupt.");
                }
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate, bool fromStartup)
{
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            // Will try to rescan it if zSUV upgrade is active.
...
CWallet* CWallet::CreateWalletFromFile(const std::string walletFile)
{
    ...
    if (!zwalletInstance->GetMasterSeed().IsNull()) {
        //Inititalize zSUVWallet
        uiInterface.InitMessage(_("Syncing zSUV wallet..."));
