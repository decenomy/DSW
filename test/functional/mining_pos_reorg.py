...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected ESK and zESK supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zESKsupply']:
                assert_equal(node['zESKsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check ESK and zESK supply at the beginning
        # ------------------------------------------
        # zESK supply: 2 coins for each denomination
        ...
        # ESK supply: block rewards minus burned fees for minting
        ...
        # Verify that ESK and zESK supplies were properly updated after the spends and reorgs
        self.log.info("Check ESK and zESK supply...")
