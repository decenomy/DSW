...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected OWO and zOWO supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zOWOsupply']:
                assert_equal(node['zOWOsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check OWO and zOWO supply at the beginning
        # ------------------------------------------
        # zOWO supply: 2 coins for each denomination
        ...
        # OWO supply: block rewards minus burned fees for minting
        ...
        # Verify that OWO and zOWO supplies were properly updated after the spends and reorgs
        self.log.info("Check OWO and zOWO supply...")
