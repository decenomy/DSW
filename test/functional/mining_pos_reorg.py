...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected KYAN and zKYAN supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zKYANsupply']:
                assert_equal(node['zKYANsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check KYAN and zKYAN supply at the beginning
        # ------------------------------------------
        # zKYAN supply: 2 coins for each denomination
        ...
        # KYAN supply: block rewards minus burned fees for minting
        ...
        # Verify that KYAN and zKYAN supplies were properly updated after the spends and reorgs
        self.log.info("Check KYAN and zKYAN supply...")
