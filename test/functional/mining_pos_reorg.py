...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected DASHD and zDASHD supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zDASHDsupply']:
                assert_equal(node['zDASHDsupply'][denom], DecimalAmt(expected_zpiv[denom]))
