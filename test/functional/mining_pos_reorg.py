...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected UCR and zUCR supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zUCRsupply']:
                assert_equal(node['zUCRsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check UCR and zUCR supply at the beginning
        # ------------------------------------------
        # zUCR supply: 2 coins for each denomination
        ...
        # UCR supply: block rewards minus burned fees for minting
        ...
        # Verify that UCR and zUCR supplies were properly updated after the spends and reorgs
        self.log.info("Check UCR and zUCR supply...")
