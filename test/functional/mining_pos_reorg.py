...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected PEPS and zPEPS supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zPEPSsupply']:
                assert_equal(node['zPEPSsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check PEPS and zPEPS supply at the beginning
        # ------------------------------------------
        # zPEPS supply: 2 coins for each denomination
        ...
        # PEPS supply: block rewards minus burned fees for minting
        ...
        # Verify that PEPS and zPEPS supplies were properly updated after the spends and reorgs
        self.log.info("Check PEPS and zPEPS supply...")
