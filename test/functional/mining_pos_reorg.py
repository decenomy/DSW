...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected CFL and zCFL supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zCFLsupply']:
                assert_equal(node['zCFLsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check CFL and zCFL supply at the beginning
        # ------------------------------------------
        # zCFL supply: 2 coins for each denomination
        ...
        # CFL supply: block rewards minus burned fees for minting
        ...
        # Verify that CFL and zCFL supplies were properly updated after the spends and reorgs
        self.log.info("Check CFL and zCFL supply...")
