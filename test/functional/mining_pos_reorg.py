...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected BIR and zBIR supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zBIRsupply']:
                assert_equal(node['zBIRsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check BIR and zBIR supply at the beginning
        # ------------------------------------------
        # zBIR supply: 2 coins for each denomination
        ...
        # BIR supply: block rewards minus burned fees for minting
        ...
        # Verify that BIR and zBIR supplies were properly updated after the spends and reorgs
        self.log.info("Check BIR and zBIR supply...")
