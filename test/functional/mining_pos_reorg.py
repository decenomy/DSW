...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected SUV and zSUV supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zSUVsupply']:
                assert_equal(node['zSUVsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check SUV and zSUV supply at the beginning
        # ------------------------------------------
        # zSUV supply: 2 coins for each denomination
        ...
        # SUV supply: block rewards minus burned fees for minting
        ...
        # Verify that SUV and zSUV supplies were properly updated after the spends and reorgs
        self.log.info("Check SUV and zSUV supply...")
