...
def main():
    ...
    tmpdir = "%s/jackpot_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and jackpotd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0, keep_cache=False):
    ...
    try:
        if subprocess.check_output(["pidof", "jackpotd"]) is not None:
            print("%sWARNING!%s There is already a jackpotd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "BITCOIND" not in os.environ:
        os.environ["BITCOIND"] = build_dir + '/src/jackpotd' + exeext
        os.environ["BITCOINCLI"] = build_dir + '/src/jackpot-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `jackpot-cli help` (`rpc_interface.txt`).
